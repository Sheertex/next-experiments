...
class ExperimentExtractorPlugin {
    constructor(options = { resultFilePath: EXPERIMENTS_FILE_NAME, isDebug: false, libraryName: LIBRARY_NAME }) {
    ...
    apply(compiler) {
        ...
        compiler.hooks.normalModuleFactory.tap(PLUGIN_NAME, (factory) => {
            factory.hooks.parser.for("javascript/auto").tap(PLUGIN_NAME, (parser) => {
                parser.hooks.importSpecifier.tap(PLUGIN_NAME, (statement, source, exportName, identifierName) => {
                    if (source.indexOf(this.libraryName) !== -1 &&
        ...
        compiler.hooks.emit.tap(PLUGIN_NAME, (compilation) => {
            if (foundModulesWithExperiments.length === 0) {
                compilation.errors.push(new Error(`Did not find any .jsx or .tsx component with "import { ${EXPERIMENT_COMPONENT_NAME} } from "${this.libraryName}"`));
            }
                            ...
                            foundModulesWithExperiments.forEach((foundModule) => {
                                ...
                                if (componentWithExperiment === filepath) {
                                    ...
                                    if (experimentsByChunkName.has(chunkName)) {
                                        ...
                                        experimentsByChunkName.set(chunkName, Object.assign(Object.assign({}, prevData), { componentsWithExperiments: [
                                                ...prevData.componentsWithExperiments,
                                                componentWithExperiment,
                                            ], experimentsPayload: Object.assign(Object.assign({}, prevData.experimentsPayload), experimentsData) }));
                                    }
                                    else {
            ...
            const experiments = Array.from(experimentsByChunkName.values())
                ...
                .map((value) => {
                if (this.isDebug) {
                    return Object.assign(Object.assign({}, value), { pagePathRegex: chunkNameToPageNameRegex(value.chunkName) });
                }
                return {
