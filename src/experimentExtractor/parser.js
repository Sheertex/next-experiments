...
const { EXPERIMENT_COMPONENT_NAME, VARIANT_COMPONENT_NAME } = require('./constants')
...
function getVisitor(cache) {  
  return  {                
      JSXOpeningElement(path, state) {
          ...
          const experimentNameAttribute = openingElement.attributes.find(attr => attr.name.name === 'name');        
          ...
          path.container.children
            .filter(child => child.type === 'JSXElement' && child.openingElement.name.name === VARIANT_COMPONENT_NAME)
            .forEach(variant => {
              const variantNameAttribute = variant.openingElement.attributes.find(attr => attr.name.name === 'name');
          ...
          if (!cache[experimentNameValue]) {
            ...
            const uniqueVariantNames = new Set([...cache[experimentNameValue], ...variantNames]);
